let config: Config;
let pluginSettings: Record<string, string> = {};
...
import {
  ...
  PAGE_SERIES_EPISODE_PLAYER_QUERY,
  PAGE_MOVIE_PLAYER_QUERY,
...
function refreshEntitlementToken(videoId: string) {
  try {
    ...
    const resp = httpClient.POST(
      'https://entitlement.p7s1.io/api/user/entitlement-token',
      JSON.stringify(requestBody),
      headers
    );
...
function getCountryCode(): string {
  ...
  if (config && config.settings) {
    const countrySetting = config.settings.find((s: any) => s.variable === 'joyn_country');
...
function extractVideoSources(path: string, assetId?: string, isMovie: boolean = false): VideoSourceDescriptor {
...
function getSeriesAsVideoDetails(url: string): PlatformVideoDetails {
  ...
  const videoDetails: PlatformVideoDetailsDef = {
    ...
    thumbnails: new Thumbnails([
      new Thumbnail(
        (series.heroPortraitImage || series.images?.heroPortrait)
    ...
    cast: series.cast?.map((c: any) => c.name) || [],
    directors: series.directors?.map((d: any) => d.name) || [],
    writers: series.writers?.map((w: any) => w.name) || [],
    productionCompanies: series.productionCompanies?.map((p: any) => p.name) || [],
